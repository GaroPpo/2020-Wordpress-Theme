<?php
function pixelwars__wp_title( $title, $sep ) {
	global $paged, $page;
	if ( is_feed() ) {
		return $title;
	}
...
add_filter( 'wp_title', 'pixelwars__wp_title', 10, 2 );
...
function footer_social_media() {
	$link = array("WordpressTheme2020_social_link_1",
  ...
  $name = array("WordpressTheme2020_social_link_1_name",
  ...
  foreach (array_combine($link, $name) as $links => $names) {
		...
		if (!empty($link_show) && !empty($name_show)) {
			echo '<li><a target="_blank" href="'.$link_show.'">'.$name_show.'</i></a></li>';
		}
	}
}
